...
Out of **31,413** unique users, the model flagged **3,540** users (11.27%) as exhibiting highly suspicious troll-like or astroturfing behavior.
...
## 6. Deep Dive: Granular Political Targeting
By mapping the flagged comments back to their raw, detailed database paths, we can see exactly which sub-categories and specific political events attract the most astroturfing.

![Detailed Paths](./detailed_paths.png)

As shown above, specific political events (like the Austrian Presidential Election `bundespraesi` and international crises in `Tuerkei` or `Syrien`) are significantly more targeted than generic news categories. This confirms that professional writers are actively deployed during specific political events.

## 7. Deep Dive: Non-Political Targeting
To understand if professional writers also target non-political subjects, we filtered out obvious political keywords (like Inland, International, Politik, Flucht, Asyl) and analyzed the remaining targeted paths.

![Detailed Non-Political Paths](./detailed_paths_nonpol.png)

This reveals that astroturfing outside of politics is heavily concentrated in the **Economy (Wirtschaft)**, **Technology (Web/ITBusiness)**, and **Sports (Fussball/Bundesliga)**. This suggests that corporate PR accounts are deployed to artificially boost sentiment in articles about specific companies, tech products, or local sports teams.
