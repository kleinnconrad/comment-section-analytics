...
def run():
    ...
    print("Fetching raw posts for professional writers...")
    preds.rename(columns={'user_id': 'ID_User'}, inplace=True)
    flagged_posts = preds[preds['y_pred_any'] == 1].copy()
    
    print("Classifying intent for 146,000+ flagged comments. This will take a moment...")
    ...
    print(f"Total posts with 'left' intent: {(flagged_posts['Intent'] == 'left').sum()}")
    print(f"Total posts with 'right' intent: {(flagged_posts['Intent'] == 'right').sum()}")
    ...
    print(f"Successfully classified {len(final_spectrum)} professional writers into strict Left/Right spectrums based on >= {POLITICAL_MAJORITY_THRESHOLD*100}% majority.")
    ...
    print("Generating Political Spectrum Target Chart (TOTAL)...")
    ...
    for year in years:
        ...
        plt.title(f'Targets of Highly Partisan Professional Writers ({year})\n(Users with >{int(POLITICAL_MAJORITY_THRESHOLD*100)}% Ideological Consistency)', fontsize=14, pad=15)
        plt.xlabel('Number of Flagged Comments')
        plt.ylabel('Detailed Newspaper Path')
        plt.legend(title='Assigned Political Spectrum')
        plt.tight_layout()
        plt.savefig(f'results/prof_writing_analytics/political_spectrum_targets_{year}.png', dpi=300)
        plt.close()
        
        md_content += f"## {year}\n![{year}](./political_spectrum_targets_{year}.png)\n\n"

    print("Writing chronological report...")
    with open('results/prof_writing_analytics/political_timeline.md', 'w', encoding='utf-8') as f:
        f.write(md_content)
        
    print("Political intent analysis complete!")
